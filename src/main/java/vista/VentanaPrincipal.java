...
public class VentanaPrincipal extends javax.swing.JFrame {
    ...
    private void bAnalizarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_bAnalizarActionPerformed
        ...
        try {
            // Mostrar qué archivo se está analizando
            consola.append("Analizando: " + archivoActual.getSelectedFile().getName() + "\n");
            consola.append("==================================\n");

            FileReader lectorArchivo = new FileReader(archivoActual.getSelectedFile().getPath());
            Lexico lexer = new Lexico(lectorArchivo);

            lexer.tabla_de_simbolos.clear();
            ...
            consola.append("\n--- TABLA DE SÍMBOLOS ---\n");
            if (lexer.tabla_de_simbolos.isEmpty()) {
                consola.append("(No hay símbolos)\n");
            } else {
                consola.append(String.format("%-20s %-12s %-12s %-12s %-12s\n",
                        "Nombre", "Token", "Tipo", "Valor", "Long"));
                consola.append("------------------------------------------------------------\n");

                for (String[] fila : lexer.tabla_de_simbolos) {
                    consola.append(
                            String.format("%-20s %-12s %-12s %-12s %-12s\n",
                                    (fila[0].length() > 20 ? fila[0].substring(0, 17) + "..." : fila[0]),
                                    fila[1],
                                    fila[2],
                                    (fila[3].length() > 12 ? fila[3].substring(0, 9) + "..." : fila[3]),
                                    fila[4]
                            )
                    );
                }
            ...
            consola.append("\nTotal de símbolos: " + lexer.tabla_de_simbolos.size() + "\n");

        } catch (FileNotFoundException ex) {
            consola.append("ERROR: No se encontró el archivo\n");
        } catch (Exception e) {
        consola.append("=== ERROR DETALLADO ===\n");
        consola.append("Tipo: " + e.getClass().getName() + "\n");
        consola.append("Mensaje: " + e.getMessage() + "\n");

        // Stack trace completo
        StringWriter sw = new StringWriter();
        PrintWriter pw = new PrintWriter(sw);
        e.printStackTrace(pw);
        consola.append("Stack trace:\n" + sw.toString() + "\n");

        // Muestra las últimas reglas ejecutadas
        consola.append("\nÚltimas reglas ejecutadas:\n");
        consola.append(je.toString() + "\n");
    }
